...
[54](54-bes-dil.md)'te üç dil eklerken müdavimlerin meslek satırlarını ve
...
için yazıldı ([44](44-magaza-metinleri.md)). Karakter sayıları **elle
yazılmıştı ve ikisi yanlıştı** (60 diyordu, 61'di). `tools/content/check_magaza.py`
...
iki denetçinin arasında on dosya. `tools/content/diller/` açıldı; ad kalıbı
...
Üç denetçiyi (`audit_content.py`, `check_lisans.py`, `check_urp.py`) topic
klasörlerine taşımayı **denemedim**: on beş belge atıfını bayatlatırdı, kazancı
yoktu.

### Bunun yerine taşımayı güvenli yapan bir denetim

`tools/check_belgeler.py`: her göreli bağlantı var olan bir dosyayı gösteriyor
mu, ve her numaralı belge dizinde anılıyor mu. İlk koşusunda **dokuz kırık
bağlantı** buldu — biri [54](54-bes-dil.md)'te benim yazdığım yanlış numaraydı,
