...
**Review and measurement** — [22](22-answers-and-direction.md) questions and direction · [37](37-four-agent-review.md) the four-agent review · [43](43-review-and-measurement.md) where the measurement misled us · [45](45-design-review.md) the five-agent design pass · [49](49-unreachable-mechanics.md) mechanics that never reach the player · [57](57-end-to-end-audit.md) the end-to-end audit: the busy slot was the empty one · [58](58-visual-review.md) the visual round: a blank icon, a shadowless floor, an inverted sky · [59](59-kitchen-equipment-and-models.md) the kitchen you can see: equipment, models, animation · [60](60-palette-and-light.md) the palette and the light · [61](61-scale-and-silhouette.md) one scale, and two kitchens that are not the same shape · [62](62-service-agency.md) the service phase: what the owner does · [46](46-shipped-binary.md) the shipped binary, link.xml
...
| **[60-palette-and-light.md](60-palette-and-light.md)** | **The palette and the light: the whole picture lived in the bottom third of the value range - nothing indoors reached 35% and the sky stood darker than the ground. Both palettes lifted and desaturated, the kitchen given four materials instead of one grey, and the lever that actually held the floor down turned out to be the LIGHT: a roofless room sits in its own shadow, and lightening a floor from 0.40 to 0.60 moved it from 14% to 17%** | **Current** |
| **[61-scale-and-silhouette.md](61-scale-and-silhouette.md)** | **One scale: the hall furniture had been brought down to the figure's size and the kitchen had not, so one building held furniture drawn to two scales - a cook at a counter level with their shoulders, a washer who could not reach the basin. The ratio was read off the hall (0.75) rather than invented, and the stations take it on the ROOT transform so the measured packing follows. Plus the one object above the line that is not the same in both kitchens: a heat-lamp pass against a hanging copper pot rail** | **Current** |
| **[62-service-agency.md](62-service-agency.md)** | **What the owner does while the day runs. The reflex answer - add levers - was wrong twice over: a patient bot finds only 0.8 uses a day, AND an eager player's whole day budget is gone at 0:45 of an eight-minute day. Spreading beats enlarging and it is not close: a six-charge budget spends 5.18 and loses 59 parties, a three-charge regenerating pool spends 3.08 and loses 53. Plus last orders, the sink call during service, a target for the third verb, and the loan button that was decided once per session** | **Current** |
| **[63-calibration-audit.md](63-calibration-audit.md)** | **What the calibration was actually measuring. Every sweep copied a stale cook capacity over a corrected one, so a whole candidate table described a cook who serves more guests than the menu allows - and the obvious repair closed a feedback loop that drifted the number one notch per sweep. The guard for all of it existed, ran 52 assertions, and had never been called. The corrected sweep re-derives the setting already in the tree, and settles that the growth multiplier is NOT a calibration problem: nine rates were tried and the two that pass it let cheap ingredients beat good play by 38%. Then the complaint that appeared at EVERY rate turned out to be neither a design fault nor a wrong check but a bot that asked once and gave up - worth six points on seven rows of nine, which is not noise but a slope. With it fixed the surface has a single minimum and a six-point margin, and exactly one failure is left - which turned out to be the check dividing the till instead of what the campaign earns: with the 8,000 stake taken off, fast food growth is 2.56 (inside the band) and Turkish is 35 (a non-expander who earns 305 coins in sixty days). The check had both cuisines backwards at once. Then "let it not earn zero": four levers measured one at a time, two reverted, two kept - the bot was switching every side and dessert on every morning, and the recommendation was padding a buffer with a safety margin - and the verification came back clean in both cuisines for the first time** | **Current** |
| **[64-what-the-game-is-about.md](64-what-the-game-is-about.md)** | **Three readers, two questions: what is this game about and where does it contradict itself. The game is about the morning while the marketing is about the service; the tab said "builds loyalty" and the loyalty was wired to the market screen instead of the door (fixed, measured, guarded); the crew axis gives its best mark to the overstaffer; from week 4 the reference player's campaign is the same day repeated; and the contradiction to fix first is that the game shows seven axes while its own tooling reads the till. Then five of the six items were done the same day: the calibration scores the plaque, the crew axis measures the roster against the peak, a defaulter is remembered by name, the calendar announces the tab and the seasons, and firing costs a week's wage - which the bots then had to learn, because a rule tuned when a sacking was free fired forty-six times per eight campaigns** | **Current** |
