...
Yemek grupları **mutfağa özel**. [13-veri-semalari.md](13-veri-semalari.md) bunu kasıtlı tasarlamış: örnek yemek `kuru_fasulye` ve grubu `sulu`, örnek arketipin tercihi `{ "sulu": 0.6, "pilav": 0.25, "corba": 0.15 }`.
...
**Yükleme aşamasında** (`ContentSetLoader`) — aynı dört kontrol, çünkü [23-cekirdek-sozlesmesi.md](23-cekirdek-sozlesmesi.md) §9.1 içerik geçersizse oyunun **açılmamasını** şart koşuyor. Sessiz varsayılan yok.
...
Farkların hepsi zirvenin keskinliğinden çıkıyor. Türk lokantasının öğle dilimi günün %48'i; o pencerede fazla kadro **işe yarıyor**, fast food'un daha düz gününde ise sadece maaş oluyor. Bu, [10-mutfak-kimligi.md](10-mutfak-kimligi.md)'nin istediği "mutfaklar farklı oynansın" hedefinin ölçülmüş ilk kanıtı.
...
- [27-zaman-modeli.md](27-zaman-modeli.md) §3.3 zirve tablosu tatlıya 0,08 eş zamanlı tabak veriyor — o satır boşa çalışıyordu
- [32-ekipman-ve-yeniden-denge.md](32-ekipman-ve-yeniden-denge.md)'de yazdığım **tatlı istasyonunun ekipman yükseltmesi** satın alınabiliyor ama hiçbir işe yaramıyordu
...
[13-veri-semalari.md](13-veri-semalari.md) arketip başına bir sipariş tercihi tasarlamış:

```json
"orderPreference": { "sulu": 0.6, "pilav": 0.25, "corba": 0.15 }
```

Simülasyon bunu okumuyor. Onun yerine sabit bir model kullanıyor: **ana yemek kesin, %30 yan, %40 içecek** ([12-ekonomi.md](12-ekonomi.md) `order` bloğu). Menü rolleri o modeli ikinci mutfakta çalışır hâle getirdi ama tasarlanan modeli uygulamadı.
