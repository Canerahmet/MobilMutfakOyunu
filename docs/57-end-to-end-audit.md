...
### The package, rebuilt and opened from the inside

`tools/android/device.ps1` needs a phone and there is no test phone, so the
second gate cannot be closed here. The **package** can be, and it was 15
September old: rebuilt at 90.6 MB with zero warnings, then read the way
[46](46-shipped-binary.md) reads one - off the file, not off the build script.

| checked | reading |
|---|---|
| package name | `com.ahmetakar.lokanta`, versionCode 1, versionName 0.1.0 |
| SDK floor / target | `minSdk 29`, `targetSdk 36` (the Play threshold is 35) |
| 64-bit | seven `.so` files, **all** `arm64-v8a`, nothing else |
| native packaging | all seven stored UNCOMPRESSED - the 108 MB -> 84 MB install fix still holds |
| 16 KB pages | every `PT_LOAD` segment `p_align = 0x4000`, read out of the ELF headers |
| permissions | one, `DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION`, generated by the toolchain and invisible to the player |
| backup / debug | `allowBackup=false`, no `android:debuggable` |
| licence texts | all four - Rubik OFL, Noto OFL, Kenney CC0, the engine components - are inside the bundles |
| the five languages | the Chinese and Arabic string tables are in the package, not only in the repository |

The last two rows are worth the trouble because neither is a file in the
package: Unity serialises them into bundles, so a listing of entry names finds
nothing and a search of the bytes finds everything. **My first search reported
the Kenney licence missing**, and the needle was wrong - the file says "CC0" and
"Creative Commons" and never the phrase I searched for. A licence check that
looks for the wrong string tells you the licence is absent in exactly the voice
it uses to tell you the truth.
...
BuildPlayer.Android                90.6 MB, 0 warnings, arm64 only
```
