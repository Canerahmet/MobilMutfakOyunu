...
Resources/ses/ klasorune atmak kod degisikligi istemiyor, lisans metni
...
Ayrica iki ayri ATIF defteri vardi (vendor/ATIF.md ve Art/ATIF.md) ve
...
Denetlenenler:
  ...
  2. O lisans TICARI kullanima aciksa bilinen bir lisans mi (CC0, OFL,
     MIT, Apache, CC-BY).
  3. Resources/ses/ altinda ses dosyasi varsa lisansi da var mi.
  4. Art/ATIF.md her klasoru aniyor mu.
...
SES = os.path.join(ROOT, "unity", "Assets", "Lokanta", "Resources", "ses")
LISANS = os.path.join(ROOT, "unity", "Assets", "Lokanta", "Resources", "lisans")
ATIF = os.path.join(ART, "ATIF.md")

# Varlik TASIMAYAN klasorler: uretilmis ya da projenin kendi ciktilari.
URETILEN = {"Malzeme", "Prefab", "Animator", "Mesh"}
...
def main():
    ...
    atif = read(ATIF)
    if not atif:
        sorun.append("Art/ATIF.md yok - atif defteri yayin oncesi zorunlu")
    ...
    for ad in sorted(os.listdir(ART)):
        ...
        if ("| " + ad + " |") not in atif:
            sorun.append("Art/%s: ATIF.md klasor eslemesinde satiri yok" % ad)
    ...
    if sesler:
        ...
        if "ses" not in atif.lower():
            sorun.append("Resources/ses/ dolu (%d dosya) ama ATIF.md'de ses "
                         "bolumu yok" % len(sesler))
        if not lisans_metni:
            sorun.append("Resources/ses/ dolu ama Resources/lisans/ bos")
        satir.append("  %-10s %d dosya" % ("ses", len(sesler)))
    else:
        satir.append("  %-10s yok (risk sifir)" % "ses")
